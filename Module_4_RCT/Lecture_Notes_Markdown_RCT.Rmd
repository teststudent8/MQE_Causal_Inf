...
```
 ...
 Why does adding excoriates add precision? Think about the formula for the variance/standard error of our estimator:
...
There are many instances where SUTVA does not hold. A classic example of SUTVA not holding is the case of vaccines. If $D_i$ represents inoculation of unit $i$ with the measles vaccine, and $Y-i$ represents whether unit $i$ gets the measles, clearly $Y_i(D_i)$ depends on all of the $D$'s, not just $D_i$. For example, if all the units are vaccinated, except $i$ then $Y_i(1)-Y_i(0)=0$, ie the vaccine has no treatment effect on unit $i$, since there is no one for $i$ to catch the measles from. 
