...
Suppose you are a bank interested in predicting customer's ability to repay student loans. You have a subset of CPS data on earnings and the number of years spent in education. You have a subset of CPS data that includes earnings and years in education.
...
\center ![]("C:\Users\Claire\Dropbox\MQE_CI_Fall2020\Lecture Notes\scatterintuitioncomic.png"){width=75%}
...
\center ![]("C:\Users\Claire\Dropbox\MQE_CI_Fall2020\Lecture Notes\corcauscomic.png"){width=75%}
...
If I estimate $Income_i=\tilde{\alpha}+\tilde{\tau}college_i+\epsilon_i$ we saw that  $E[\tilde{\tau}]\neq\tau$ due to selection bias. 
...
Then
$$
earnings_i=\hat{\alpha}+\hat{\tau}+\hat{\gamma}hhinc_i+\epsilon_i
$$
...
I am interested in how health related to income. Using my CPS sample of working age adults I estimate
$$
Income_i=\beta_0+\beta_1Health_i+\epsilon, 
$$
where heath is an respondents subjective assessment of their health with 1 being very health and 5 being very unhealthy. 
...
agg_data<-read_dta(here("data/IA_MI_merge040504.dta"))
nrow(agg_data)
