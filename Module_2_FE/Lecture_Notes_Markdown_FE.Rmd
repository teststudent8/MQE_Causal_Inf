...
Our ability to control for important omitted variables increases substantially when we can better control for unobserved confounders. There are a number of strategies econemetricians use to help control for unobservable confounds. These strategies require data that has a time or a cohort dimension in order to control for unobserved but fixed omitted variables, aka **fixed effects**.
...
Unique observations must be identified by both the individual and time dimensons, thus the equation above requires the addition of time subscripts, to uniquely identify an observation. 
...
We are now working with __panel data__, which  consists of repeated observations of the same unit at multiple points in time. Panel data can be balanced, if you observe every unit the same number of times, or unbalanced, if some units are observed more often than others.^[In the event that you are working with unbalanced data, attention should be given to why some units are oserved more frequently then others as there may be a selection process occuring.]

\subsection{Fixed Effects}

Recall that we know we can control for the effect of being female on wages by adding an controling for the female indicator in the regression as follows. I was able to do this because I had multiple Female observation and multiple non-female observations. 
...
\begin{equation}
\begin{split}
Income_{it}=\beta_0+\beta_1Schooling_{it}+\beta_2Female_i+\beta_{a1}Indiv1_i+\beta_{a2}Indiv2_i+...+\beta_{aN-1}Indiv(N-1)_i+\epsilon.
\end{split}
\end{equation}

Notes: 
\begin{itemize}
\item These dummy variables only have an i subscript as the value of this dummy indicator only varies accross individuals, and not across time. (Question: What is the implied assumption if Female only has an i subscript?). 
\item Notice that only $(N-1)$ individual dummies are added. Why? Just as with any other variable, we must have an omitted cotegory to avoid the problem of multicollineariy. 
\end{itemize}
...
Notice that given the data I can add an additional control that may be important. Suppose I am concerned that incomes in my data were severeley affected by the financial crisis in 2008. Just as above, I can control for any common effect a particular year had on all the individuals by adding year fixed effects ($\tau_t$) as specified below,
...
Suppose you are a principle of a small school composed of four classrooms. You have just implemented a new option available to teachers for students to spend some small group reading time with a paraeducatior. You would like to know how this reading time is affecting reading scores.  You have data for ten students in each class that tells you the class the student is in, whether they participated in small group reading and their reading score. 
...
Finally, I genrerate the simulated outcomes. The true treatment effect is set to 15. Notice that I am simulating a situation in which students in classrooms 1 and 2 have much higher reading scores then those in classrooms 3 and 4. 
...
To build intuition about how fixed effects work, it might be helpful to think about fixed effect as the **within estimator**, because it identifies $\beta$ using within-unit variation. In the example of our classroom reading scores, when estimatimating using the classroom fixed effects, we only using the  variation that exists __within the classroom__ to estimate the treatment effect. This is the equivalent of "correcting" our data by demeaning each observation using it's classroom mean, so that the corrected data represents deviations from the classroom mean. Our fixed effect estimation is 

$$
y_{ci}=\beta_1x_{ci}+\kappa_c+\epsilon_{ci}
$$
For each class, the average accross the students is
...
We still get this puzzling result. We could continue to add more controls but there are so many variables about a city that could be correlated with both unemployment and crime that it seems unlikely we could observe them all. But what id we can **capture all unobserved, time invariant factors** about a city that might affect crime rates? If I use the data for 1987 and 1982, I can add city **fixed effects,**($\alpha_i$) to the regression I estimated above,
...
Suppose I am concerned about how national factors could be affecting all cities simultaneously. I can add a year fixed effect, ($\tau_t$) to control for these

$$
crmrte_{it}=\beta_0+\beta_1unemp_{it}+\beta_2area_i+\beta_3west_i+\beta_4offarea_{it}+\beta_5lawexp_{it}+\beta_6pcinc_{it}+\alpha_i+\tau_t+\epsilon
$$
...
So can I enterpret the coefficient on this last regression as causal? We need to think carefully about what we have controlled for. The city fixed effects control for any time invariant factors that always affect the crime rates in a city in a similar way. This would be things like geography, street layouts, weather... The time fixed effects controls for any patterns that are common to all cities in a given year. This would be things like national policies such as the war on drugs, national interest rates... In addition to this we are also controling for some observable time varying variables: officers in an area, law enforcement expenditures per capita and income per capita. So are these estimates causal? What kind of ommitted variables should we still be concerned about? Any variable that changes within a city accross years and that is correlated with both unemployment and crime rates could still be biasing our results. This coule be things like school funding, criminalization of marijuana, housing costs... to name just a few.

Suppose I get ambition and want to control for all these factors as well. I decide I am going to generate a city-by-year fixed effect, ($\gamma_{it}$),  to control for these time variant omitted variables. I estimate,  
...
What happened?!? Because the variation in my outcome variable is at the city-by-year level, incluting a city-by-year fixed effect absorbe all of the identifying variation, making it impossible to estimate the effect of any of the other variables in my model. Thus I cannot include this type of fixed effect with this data set. If I had data on neighbourhood crime rates and neighbourhood unemployment I could. 
